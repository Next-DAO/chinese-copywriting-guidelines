...
推荐多用短句，使用多个短句，去组合成原来的长句，这样能显著降低阅读压力，提升阅读方的理解速度。
在不影响表达准确度和效果意图的前提下，少用形容词和连词，尽量把句子缩减到最短。你，就是下一个海明威。

正确：

>本产品适用于多种体系结构。无论是由一台服务器（单一节点结构），还是由多台服务器（并行处理结构）进行动作控制，均可使用。
